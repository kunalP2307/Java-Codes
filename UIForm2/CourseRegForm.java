...
public class CourseRegForm extends Frame implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == bSubmit) {
			...
			String mob = tMobNo.getText();
			String coursesSelected = "";
		
			for(int i=0; i<5; i++) {
				if(cBoxCourses[i].getState() == true)
					coursesSelected += courses[i] + ", "; 
			}
		
		
		
		Checkbox cBox = cBoxGender.getSelectedCheckbox();
		String gender = cBox.getLabel();
		
		String address = tAddress.getText();
		
		new ShowRegDetails(fName,lName,mob,coursesSelected,gender,address);
		}
	}
